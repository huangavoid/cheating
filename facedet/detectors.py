...
class Detector:
	def detect(self, src):
		raise NotImplementedError("Detector MUST implement the detector(self, src).")

class SkinDetector(Detector):
	""" 
	common color thresholding rules for the RGB/YcrCb/HSV color space.
	"""
	def _R1(self,BGR):
		B = BGR[:,:,0]
		G = BGR[:,:,1]
...
class CascadedDetector(Detector):
	"""
	Uses the OpenCV cascades to perform the detection.
	Returns the Regions of Interest, where the detector assumes a face. 
	"""
	def __init__(self, cascade_fn="/home/voise/Documents/cheating/haarcascade_frontalface_alt2.xml", scaleFactor=1.2, minNeighbors=5, minSize=(30,30)):
...
class SkinFaceDetector(Detector):
	"""
	1. Uses the CascadedDetector to detect faces from image
	2. Uses the SkinDetector to accept only faces over threshold (ignored for grayscale images).
	"""
	def __init__(self, threshold=0.3, cascade_fn="/home/voise/Documents/cheating/haarcascade_frontalface_alt2.xml", scaleFactor=1.2, minNeighbors=5, minSize=(30,30)):
...
if __name__ == "__main__":
	# script parameters
	if len(sys.argv) < 2:
		print "<USAGE>: python detector.py <src_img> <dst_img>"
		sys.exit()
